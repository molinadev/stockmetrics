...
export async function GET(request: Request) {
  ...
  const quotes=await marketDataProvider.getMultipleQuotes(symbols.map(providerSymbol)); const currencies=[...new Set(holdings.map(h=>h.costCurrency))]
  const fx=Object.fromEntries(await Promise.all(currencies.map(async c=>[c,await marketDataProvider.getFxRate(c,'EUR')] as const)))
  const rows=holdings.map(h=>positionFor(h,quotes[providerSymbol(h.symbol)] ?? {symbol:h.symbol,price:null,previousClose:null,change:null,changePercent:null,currency:null,name:null},fx[h.costCurrency] ?? 1))
