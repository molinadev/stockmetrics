...
export function positionFor(holding: Holding, quote: Quote, costFx: number, quoteFx = costFx) {
  const price = quote.price; const previous = quote.previousClose
  const cost = holding.quantity * holding.averageCost * costFx
  const value = price == null ? null : holding.quantity * price * quoteFx
  const openPnl = value == null ? null : value - cost
  return { ...holding, quote, value, cost, openPnl, openPnlPct: value == null || !cost ? null : (value / cost - 1) * 100, todayPnl: price == null || previous == null ? null : (price - previous) * holding.quantity * quoteFx, todayPnlPct: price == null || !previous ? null : (price / previous - 1) * 100 }
}
